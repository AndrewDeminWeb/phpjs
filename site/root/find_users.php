  ...
  <div class="container" >
        ...
        <p><input type="submit" value="Вернуться на главную страницу" name="submit" class="btn-oval" style=" color: white; font-family: 'Monotype Corsiva'; font-size: 30px;"></p>
			...
			<div class="row" style=" color: white; font-family: 'Monotype Corsiva'; font-size: 30px;">
				<form action="" method="post" style="margin-bottom: -140px; margin-top: 20px; font-size: 30px;">
	    		<p>Введите логин искомого человека</p>
	    		<p><input name=f_name id='login' class="textovii" onkeyup="checkParams()" style="margin-top: 25px; font-size: 25px;"></p>
	    			  	<input type="submit" value="Найти фотографии" name="submit" class="btn-oval" style="margin-top: 25px; font-size: 25px;">
	        </form>
				</div>
				</div>
	...
	$idea=0;
	    ...
	    exit;}
					    ...
					    $query = "SELECT user_id FROM siteusers WHERE login = '$name'";
					$result = mysqli_query($link, $query);
	while($row = mysqli_fetch_row($result)){
		$id=$row[0];
	}

		$query = "SELECT * FROM photo WHERE user_id = '$id' AND view='1'";
		$result = mysqli_query($link, $query);
